...
class StorageService:
    ...
    def upload_evidence(self, file_bytes: bytes, filename: str, content_type: str) -> str:
        """Upload supporting inspection evidence and return its storage URL."""
        client = self._require_client()
        key = f"evidence/{uuid.uuid4()}/{filename}"
        client.storage.from_(self.label_bucket).upload(key, file_bytes, file_options={"content-type": content_type})
        return client.storage.from_(self.label_bucket).get_public_url(key)
