...
def list_reports(
    ...
    try:
        ...
        if role == "admin":
            ...
            cur.execute("SELECT COUNT(*) AS total FROM reports r JOIN users u ON u.id=r.user_id WHERE u.state=%s", (current_user.get("state"),))
            total = (cur.fetchone() or {}).get("total", 0)
            cur.execute("SELECT r.* FROM reports r JOIN users u ON u.id=r.user_id WHERE u.state=%s ORDER BY r.created_at DESC LIMIT %s OFFSET %s", (current_user.get("state"), page_size, offset))
        else:
...
def generate_report(
    ...
    try:
        ...
        # Access is scoped to the requester's own history, or to the supervisor's state.
...
async def download_report(
    ...
    # Access control
    allowed = current_user.get("role") == "admin" or str(report["user_id"]) == str(current_user["id"])
    if current_user.get("role") == "supervisor":
        conn = get_db_connection()
        try:
            cur = conn.cursor()
            cur.execute("SELECT state FROM users WHERE id=%s", (report["user_id"],))
            allowed = (cur.fetchone() or {}).get("state") == current_user.get("state")
        finally:
            conn.close()
    if not allowed:
        raise HTTPException(
